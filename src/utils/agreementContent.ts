 ...
 * 用户服务协议 HTML
 */
export function getUserAgreementHtml(): string {
  const content = `
    <h3>1、导言</h3>
    ...
    <p>• 10.3 你知悉、理解并同意，为持续改善并为您提供更好的服务，你通过"火箭体育"软件及相关服务上传发布、传输或传播的内容（包括但不限于文字、图片、图像、音频、视频和/或音频中的音乐、声音、对话等），授权公司及其关联公司、控制公司、继承公司一项全球范围内、免费、非独家、可再许可（通过多层次）的权利（包括但不限于复制权、信息网络传播权、改编权、汇编权、修改权、翻译权、制作衍生品、表演和展示等权利），使用范围包括但不限于在当前或其他网站、应用程序、产品或终端设备等。</p>
